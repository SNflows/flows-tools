...
def get_basename_with_suffix(filename: Path, suffix=""):
    return Path(str(filename)[: str(filename).rfind("".join(filename.suffixes))] + suffix + "".join(filename.suffixes))


def parse():
    parser = argparse.ArgumentParser(
        prog="FixTemplateSubHeaderforLCOGT",
        description=(
            "Attach header and extra dimensions from second                                      file to the first,"
            " while checking to make sure WCS is identical.                                     If not, uses WCS"
            " offirst file."
        ),
    )
